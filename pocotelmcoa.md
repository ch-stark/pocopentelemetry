The last tutorial treated Tempo as the product. This one is built from [multicluster-observability-addon](https://github.com/stolostron/multicluster-observability-addon): MCOA installs operators on spokes and copies hub stanzas; it does not create Tempo or Loki.
...
MCOA is the control plane for **collection and fan-out**. Tempo/Loki stay outside the addon; you only plug them in as the stanza exporter/output, which is how [`hack/addon-install`](https://github.com/stolostron/multicluster-observability-addon/tree/main/hack/addon-install) is written.
